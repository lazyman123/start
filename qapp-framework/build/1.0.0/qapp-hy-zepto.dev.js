...
;(function() {
    ...
    QApp.addPlugin('zepto', DEFAULT_OPT, function(view, options) {
        var _ready = view.options.ready;

        if (_.isFunction(_ready)) {
            view.options.ready = function () {
                var root = Zepto(this.root);
                _ready.call(this, root);
            };
        }
            ...
            if (view.options.bindActions) {
                _.each(view.options.bindActions, function(key, process) {
                    ...
                    if (_.isFunction(process)) {
                        Zepto(view.root).delegate(action, eventType, function (e) {
                            return process.call(view, e);
                        });
                    } else {
                        Zepto(view.root).delegate(action, eventType, function (e) {
                            if (_.isFunction(view[process])) {
                                return view[process].call(view, e);
                            }
                        });
                    }
                });
...
(function() {
    ...
    function initHy() {
        ...
        Hy = {
            ...
            invoke: function(key, callback, param, timeout) {
                _logger.debug( '调用桥', key, '参数为', param);
                var fired = false;
                if (Hy.exists) {
    ...
    function refreshNativeHeader(navOpt) {
        if (sniff.hyApp) {
            _logger.debug( '设置 Native Header 展现形式, 参数为', navOpt);
    ...
    function listenerNavClick(root) {
        Hy.onNavClick(function(data, responseCallback) {
            _logger.debug( '监听到 Native Header 点击事件', '按钮类型为', data.type);
            var el,
                responseData = {};
            ...
            _logger.debug( responseData.data ? '阻止 Native 默认行为' : '不阻止 Native 默认行为');
    ...
    function initFilter() {
        ...
        QApp.router.addOpenFilter(function(args) {
            if (args.length && args[0]) {
                ...
                _logger.debug( '打开的 View Name 为:', viewName, '参数为:', param, 'WebView 类型为:', viewType, 'Loading 设置为', loadView);
                ...
                if (viewOptions.schema) {
                    ...
                    _logger.debug( '打开 Schema:', schema);
        ...
        if (sniff.hyApp) {
            ...
            Hy.getHistory(function(data) {
                if (data.ret && data.data.list) {
                    ...
                    _logger.debug( '获取到的 History List 为:', historyList);
                }
            });
            ...
            QApp.ready(function() {
                ...
                _logger.debug( '刷新 Native Header');
                Hy.navRefresh(navOpt);
                _logger.debug( '设置 WebView 属性');
                Hy.setWebViewAttribute({
                    name: prefixHy + '-' + Config.hybridId + '-' + originName
                });
                if (viewOptions.disableBackGestrue) {
                    _logger.debug( '禁用 ios 回退手势');
                    Hy.enableGesture({
                        enable: false
                    })
                }
                ...
                _logger.debug( '开始打开视图', viewName);
                QApp.open(viewName, {
                    param: param,
                    atBottom: true
                }, function(view) {
                    _logger.debug( '已打开视图', view.name);
                    if (view) {
                        var _complete = view.complete,
                            completeData,
                            actived = true,
                        ...
                        Hy.onHide(function() {
                            _logger.debug( 'Hide -> Deactived');
                            if (!bindShow) {
                                bindShow = true;
                                Hy.onShow(function() {
                                    _logger.debug( 'Show -> Actived');
                                    Cookie.refresh();
                                    notify('actived', view);
                                    if (!actived) {
                                        actived = true;
            ...
            _logger.debug( '执行 Qunar 客户端非 Hy 环境逻辑');
            QApp.config({
            ...
            QApp.router.on('init', function() {
                connectQunarBridge(function(bridge) {
                    _logger.debug( '隐藏 Native Header');
            ...
            QApp.on('close', function() {
                connectQunarBridge(function(bridge) {
                    _logger.debug( '关闭 WebView');
                    bridge.call("closeWeb", {}, function(e) {});
                });
            });

        } else {
            _logger.debug( '执行 Touch 环境逻辑');
            QApp.config({
                indexView: Config.indexView
            });
    ...
    _logger.debug( '初始化 QApp-Hy');
    if (sniff.qunarApp) {
        _logger.debug( '判断为 Qunar 客户端环境');
        _logger.debug( '开始监听 WebViewJavascriptBridgeReady 事件');
        document.addEventListener('WebViewJavascriptBridgeReady', function(event) {
            _logger.debug( checkDefer.state() == 'pending' ? '桥注入成功' : '');
        ...
        _.ready(function() {
            _.delay(function() {
                _logger.debug( checkDefer.state() == 'pending' ? '桥注入超时。Dom Ready 后 ' + opt.bridgeDelay + 'ms' : '');
                checkDefer.resolve();
            }, opt.bridgeDelay);
        });
    } else {
        _logger.debug( '判断为 非Qunar 客户端环境');
        _.ready(function() {
            checkDefer.resolve();
        });
    }
